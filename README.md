...
- [ ] 確認 Deployment Protection 已關閉、同學可正常訪問正式網域
- [ ] 視需求擴充英文單字庫至完整 7000 字
- [ ] 視需求擴充國文字音字形題庫（可提供教材原始檔案）
- [ ] 評估是否要新增其他科目模塊（數學、歷史等）
- [ ] 視使用狀況評估是否需要升級成資料庫 + 登入系統（跨裝置同步進度）

## 給下一個對話的操作提示

如果要接續這個專案：
1. 先請使用者提供 GitHub repo 網址或截圖確認目前檔案結構是否與本文件一致
2. 修改某個模塊的檔案時，記得先跟使用者要目前該檔案的最新內容（透過截圖或 GitHub 連結抓取），不要直接假設本文件列出的版本就是 repo 現況，因為使用者是手動上傳，可能已經做過調整
3. 使用者是用 GitHub 網頁版手動上傳/覆蓋檔案來部署（沒有 git 指令環境），所有修改都需要提供「整份檔案內容」讓使用者覆蓋貼上，不能只給 diff
